...
import { RANDOM_GAME_CONTRACT_ABI, RANDOM_GAME_CONTRACT_ADDRESS } from "../constants";
  ...
  */
  ...
  const startGame = async () => {
    try {
      ...
      const randmomGameContract = new ethers.Contract(RANDOM_GAME_CONTRACT_ADDRESS, RANDOM_GAME_CONTRACT_ABI, signer);
      setLoading(true);
      const tx = await randmomGameContract.startGame(maxPlayers, entryFee);
      await tx.wait();
      setLoading(false);
    } catch (err) {
      console.error(err);
  ...
  const joinGame = async () => {
    try {
      ...
      const randomGameNFTContract = new ethers.Contract(RANDOM_GAME_CONTRACT_ADDRESS, RANDOM_GAME_CONTRACT_ABI, signer);
      setLoading(true);
  ...
  const checkIfGameStarted = async () => {
    try {
      ...
      if (_gameStarted) {
        ...
        console.log("eFee", _game.entryFee);
  ...
  const getOwner = async () => {
    try {
      const provider = await getProviderOrSigner();
      const randomGameContract = new ethers.Contract(RANDOM_GAME_CONTRACT_ADDRESS, RANDOM_GAME_CONTRACT_ABI, provider);
        ...
        <div>
          <input
            ...
            onChange={(e) => {
              ...
              setEntryFee(e.target.value >= 0 ? utils.parseEther(e.target.value.toString()) : zero);
            }}
